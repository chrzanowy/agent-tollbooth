---
name: tollbooth
description: Use when a task needs state that outlives this session — remembering facts for future sessions, recalling what past sessions learned, checking whether a webpage changed since it was last seen, reading JS-rendered pages that plain fetch returns empty, or running code when no sandbox is available. Triggers on "remember this", "what did we decide", "has X changed", "track/monitor this page", session start in a project with stored context, and fetch results that look like an empty JS shell.
---

# tollbooth — state that survives your session

tollbooth is a local service (default `http://localhost:4402`) holding state that
outlives you: memories, page snapshots, a browser, a code runner. If its MCP tools
(`memory_store`, `memory_recall`, `watch_add`, `watch_check`, `watch_list`,
`render_extract`, `execute_run`) are available, use them; otherwise every example
below is a plain HTTP call you can make with curl.
